...
![database_netezza](https://github.com/user-attachments/assets/25972d2b-1a83-4a21-88cd-2679eed37ebd)

## Содержание

- [1. Архитектура Netezza](#1-архитектура-netezza)
- [2. Создание тестовых таблиц в Netezza](#2-создание-тестовых-таблиц-в-netezza)
    - [2.1. Предварительная подготовка. Ключи распределения](#21-предварительная-подготовка-ключи-распределения)
    - [2.2. Ключи организации](#22-ключи-организации)
    - [2.3. Создание таблиц с ключами](#23-создание-таблиц-с-ключами)
    - [2.4. Наполнение таблиц данными](#24-наполнение-таблиц-данными)
    - [2.5. Проверка распределения данных по сегментам](#25-проверка-распределения-данных-по-сегментам)
- [3. Выполнение стандартных SQL-запросов](#3-выполнение-стандартных-sql-запросов)
- [4. Использование встроенных инструментов мониторинга](#4-использование-встроенных-инструментов-мониторинга)
    - [4.1. Команда EXPLAIN](#41-команда-explain)
    - [4.2. Системные представления (qryhist, _v_qrystat)](#42-системные-представления-qryhist-qrystat)
    - [4.3. Команда SHOW PLANFILE](#43-команда-show-planfile)
- [5. Методы оптимизации запросов в Netezza](#5-методы-оптимизации-запросов-в-netezza)
- [6. Выводы](#6-выводы)
...
![изображение](https://github.com/user-attachments/assets/b3118e7c-44fd-4c3a-9e81-82ec6b0c0f26)
...
![изображение](https://github.com/user-attachments/assets/c22345b0-5c0c-46a3-8656-6cac01a48958)
...
(host_return, locus=host subject=self)
   l: Sequential Scan table "SALES" (cost=0.0..1.1 rows=1000000 width=8 conf=100)  {(SALES.PRODUCT_ID),(SALES.CUSTOMER_ID)}
      (xpath_none, locus=spu subject=self)
```
### 4.2. Системные представления (QRYHIST, QRYSTAT) 

Представления _V_QRYHIST, _V_QRYSTAT позволяют отслеживать историю выполнения запросов и текущие статистики.
...
```
Пример вывода:
![_V_QRYHIST_output_screen](https://github.com/user-attachments/assets/f0a08812-fb36-4a07-8ea7-dfa4de661388)
